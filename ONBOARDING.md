...
**The live product is at:** https://afferens.vercel.app
...
| Hosting | Vercel | Runs the app live at afferens.vercel.app |
...
│   ├── page.tsx                ← Homepage (afferens.vercel.app)
...
for m in VISION SPATIAL ACOUSTIC ENVIRONMENTAL MOLECULAR INTEROCEPTION; do
  echo "--- $m ---"
  curl -s "https://afferens.vercel.app/api/demo?modality=$m" | head -c 200
  echo ""
done
```
...
Replace `YOUR_KEY_HERE` with the key from afferens.vercel.app/dashboard after signing in.
```bash
curl -H "X-API-KEY: YOUR_KEY_HERE" "https://afferens.vercel.app/api/perception?modality=VISION"
```
...
```bash
curl -X POST "https://afferens.vercel.app/api/actuation" \
...
- Go to afferens.vercel.app/signup
...
Create an account at afferens.vercel.app. Get your API key. Call all 6 modalities. Check your token balance changes correctly. Note anything that feels broken, confusing, or missing.

**A3 — Read the docs page and audit it**
Go to afferens.vercel.app/docs. Compare every curl example on the docs page against what the API actually returns when you call it. Flag any examples that are wrong or out of date.

**A4 — Vercel redeployment verification**
The app's secret keys were recently rotated. Confirm the live app is working correctly by running all smoke tests against `afferens.vercel.app` (not localhost). If any fail, report back.
...
- No database — the MCP server just forwards requests to `afferens.vercel.app`
...
**`afferens_demo`**
- Description: `"Try the Afferens sensory API without an API key. Returns sample real-time physical perception data. Get your free API key with 10,000 tokens at afferens.vercel.app — no card required."`
- Input: `{ modality: "VISION" | "SPATIAL" | "ACOUSTIC" | "ENVIRONMENTAL" | "MOLECULAR" | "INTEROCEPTION" }`
- What it does: GET `https://afferens.vercel.app/api/demo?modality={modality}`
- Returns: raw API response

#### Perception Tools (require API key)

**`afferens_perceive`**
- Description: `"Query real-time physical perception data from Afferens. Requires an API key (free tier: 10,000 Sense Tokens at afferens.vercel.app)."`
- Input: `{ api_key: string, modality: "VISION" | "SPATIAL" | "ACOUSTIC" | "ENVIRONMENTAL" | "MOLECULAR" | "INTEROCEPTION", limit?: number }`
- What it does: GET `https://afferens.vercel.app/api/perception?modality={modality}&limit={limit}` with `X-API-KEY: {api_key}` header
- Returns: raw API response
...
- What it does: POST `https://afferens.vercel.app/api/actuation` with JSON body
- Returns: raw API response
...
- What it does: POST `https://afferens.vercel.app/api/ingest` with JSON body
- Returns: raw API response
